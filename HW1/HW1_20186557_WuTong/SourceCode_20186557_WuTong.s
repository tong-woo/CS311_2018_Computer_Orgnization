...
print_unsorted_array:                                         
   ...
   # Load buf[i] and print it
   sll   $t0, $t4, 2       # i * 4
   add   $t6, $a1, $t0       
   li    $v0, 1            # system call code for print intger
   lw   $a0, 0($t6)       # shift amount array 
   syscall                # print it
...
prepare_mergesort:
   ...
   sw   $ra,   12($sp)       
   sw   $a1,   8($sp)       # save address of buf
...
print_sorted_array:           
   ...
   sll   $t0, $t7, 2       # i * 4
   add   $t6, $a1, $t0       # address of array + offest
   li    $v0, 1            # print_int
...
merge:  
   ...
   sw $s1, 12($sp)           # save buf
   sw $s2, 8($sp)           # save right
   sw $s3, 4($sp)           # save left
   sw $s4, 0($sp)           # save register for mid
   add $s1, $zero, $a1       # $s1=array address
   add $s2, $zero, $a2       # $s2=right
   add $s3, $zero, $a3       # $s3=left
   add $s4, $zero, $a0       # $s4=mid
   add $t1, $zero, $s3       # left_i=$t1=left
...
fortst1:
   ...
   sll $t8, $t1, 2       # i*4
   la $a0, A     # load address of temporary array (neccasary if using $a0 for print statements)
   add $t8, $a0, $t8       # $t8 = address A[left_i]
